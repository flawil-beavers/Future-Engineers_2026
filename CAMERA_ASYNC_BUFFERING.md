...
The current 65 degree horizontal FOV, principal point, focal length, horizon
and ground scale are provisional values derived from the previous cropped
calibration. Physical bearing and ground-range recalibration is required before
driving.
...
1. Present an official pillar at surveyed centre and edge positions and compare
   asynchronous detections with the accepted synchronous full-FOV baseline.
2. Run the physical camera bearing and ground-range calibration; replace all
   provisional camera constants.
3. Disable the stationary auto-start override, restore the intended production
   startup mode, and repeat a motor-disabled build check.
4. Only then test asynchronous capture while Pure Pursuit drives at the
   validated low speed.
