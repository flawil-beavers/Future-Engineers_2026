...
- [ ] Upload branch `test/low-speed-drive` only with explicit user consent.
- [ ] Use a clear, level floor with at least 1.5 m of travel, keep the enable
      switch reachable, and start with the fully charged pack.
- [ ] With the enable switch LOW, send `d60`, verify manual mode is
...
- [ ] Repeat speed/steering measurements as isolated runs: disable and re-arm
...
- [ ] Repeat at `d-60`; reject unexpected direction or delayed stop.
...
- [ ] Accept the isolated controller only if neither pack shows a slow
...
- [ ] After straight tests pass, test 60 mm/s forward and reverse at steering
      -50 and +50 in a clear area; stop immediately for a stall or tight-space
      clearance risk.
- [ ] Only after both packs pass isolated tests, run one normal parked `O` test
