...
int Camera::startFrame(FrameBuffer &fb)
{
    if (this->sensor == NULL ||
        this->pixformat == -1 ||
        this->resolution == -1 ||
    ...
    if (fb.hasFixedSize() && fb.getBufferSize() < framesize)
    {
        return -1;
    }

    uint8_t *framebuffer = fb.getBuffer();
    if (framebuffer == NULL || ((uint32_t)framebuffer & 0x1F))
    {
        return -1;
    }
...
int Camera::finishFrame()
{
    ...
    const uint32_t framesize =
        frameSize() * this->sensor->getPixelReadingCycle();
    uint8_t *framebuffer = this->_framebuffer->getBuffer();
