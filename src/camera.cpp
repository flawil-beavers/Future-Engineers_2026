...
int FullFovGC2145::setResolution(int32_t resolution)
{
    ...
    // Read the complete sensor and subsample 5:1 in the GC2145. DCMI still
    // receives a compact 320 x 240 RGB565 frame containing the full view.
    int result = setWindow(0x09, 0, 0, 1616, 1208);
    result |= setWindow(0x91, 0, 0, 320, 240);
    result |= writeRegister(0x90, 0x01);
    result |= writeRegister(0x99, 0x55);
    result |= writeRegister(0x9A, 0x0E);
    return result == 0 ? 0 : -1;
}
